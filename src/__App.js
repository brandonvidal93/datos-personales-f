...
class App extends Component {
  ...
  componentDidMount() {
    ...
    if (tracking._getSuspendData() !== "") {
      ...
      // CONVIERTO LOS DOS PRIMEROS ELEMENTOS DEL ARRAY EN INT
      for (var j = 0; j < 2; j++) {
      ...
      this.setState({
        unitActual: arrayInfoSuspend[0],
        nextUnit: arrayInfoSuspend[1]
      });
      ...
      this.setState({
        unitFinal: arrayInfoSuspend[2],
        enableUnit: arrayInfoSuspend[3],
        endActivities: arrayInfoSuspend[4]
      });
    }
  }
  ...
  setSuspend = () => {
    const { unitActual, nextUnit, unitFinal, enableUnit, endActivities } = this.state;

    let dataConcat = unitActual + "#" + nextUnit + "#" + unitFinal + "#" + enableUnit + "#" + endActivities;
